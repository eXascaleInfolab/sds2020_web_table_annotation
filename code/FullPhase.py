...
LOOKUPBASED_METHOD = config('LOOKUPBASED_METHOD', cast=str)
if LOOKUPBASED_METHOD == 'Baseline_Lookup':
	import RefinedLookup as RLU
elif LOOKUPBASED_METHOD == 'Majority_Lookup':
	import RefinedLookup_proposed as RLU
else:
	print("Wrong env var for Lookup based method.")
	sys.exit()
...
def getFullPhase(T,Table_CSV_Name):
	...
	Tprime,TPrimeIsAnnotated,TPrimeAnnotation,acceptableTypes,descriptionToken,relations,SFDistanceDict = \
		RLU.RefinedLookup(T, TPrime, labelColumn, referenceColumns)

	TprimeIsAnnotated, TPrimeAnnotation = \
		AP.Annotation(T,Tprime,labelColumn,TPrimeIsAnnotated,TPrimeAnnotation,acceptableTypes,descriptionToken,relations,SFDistanceDict)
