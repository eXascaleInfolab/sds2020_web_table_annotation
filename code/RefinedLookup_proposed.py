	...
	for index, row in T.iterrows():
		...
		try:
			...
			try:
				...
				results = levenshtein_pckl[label]
				SFDistanceDict[label] = levenshtein_pckl[label]
				print("Solved with levenshtein")
				print("label:",label)
				print("Solved result:",results)
		...
		if (len(results) > 0):
			...
			try:
				...
				print("the key:",label,"not exists in Types_Dataset[label]")
				continue
